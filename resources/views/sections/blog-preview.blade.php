...
@php
    ...
    $postsList =
        isset($recentPosts) && $recentPosts->isNotEmpty()
            ? $recentPosts
            : collect([
                (object) [
                    ...
                    'image' =>
                        'https://images.pexels.com/photos/5298215/pexels-photo-5298215.jpeg?auto=compress&cs=tinysrgb&w=700&q=85',
                ],
                (object) [
                    ...
                    'image' =>
                        'https://images.pexels.com/photos/36124840/pexels-photo-36124840.jpeg?auto=compress&cs=tinysrgb&w=700&q=85',
                ],
                (object) [
                    ...
                    'image' =>
                        'https://images.pexels.com/photos/12377482/pexels-photo-12377482.jpeg?auto=compress&cs=tinysrgb&w=700&q=85',
                ],
            ]);
@endphp
                ...
                <a href="{{ isset($post->id) ? route('blog.show', $post->slug) : route('blog.index') }}"
                    ...
                    <div>
                        ...
                        <div class="relative h-48 overflow-hidden bg-[#0B332B]">
                            <img src="{{ $post->image ?? 'https://images.pexels.com/photos/5298215/pexels-photo-5298215.jpeg?auto=compress&cs=tinysrgb&w=700&q=85' }}"
                                loading="lazy" alt="{{ $post->title }}"
                                class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            <span
                                class="absolute top-3 left-3 bg-[#0B332B]/90 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-bold text-[#F5850F] border border-white/10">
                                {{ $post->category }}
                            </span>
                        </div>
                        ...
                        <div class="p-6 space-y-3">
                            <h3
                                class="font-display font-bold text-[#0B332B] text-lg sm:text-xl group-hover:text-[#F5850F] transition-colors leading-snug">
                                {{ $post->title }}
                            </h3>
                        </div>
                    </div>
                    ...
                    <div
                        class="px-6 pb-6 pt-4 border-t border-[#0B332B]/8 flex items-center justify-between text-[11px] text-[#2A2A2A]/60 font-medium">
                        <div>
                            <span>{{ is_string($post->published_at) ? $post->published_at : $post->published_at?->format('M d, Y') ?? 'Recent' }}</span>
                            <span class="mx-1.5">·</span>
                            <span>{{ $post->read_time }}</span>
                        </div>
                        <span class="text-xs font-bold text-[#0B332B] group-hover:text-[#F5850F] transition-colors">Read
                            Article →</span>
                    </div>
