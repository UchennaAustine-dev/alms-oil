  ...
  <div class="max-w-5xl mx-auto px-4 sm:px-8 lg:px-12">
    ...
    <div class="mb-12 sm:mb-16">
      <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full
                  bg-white border border-[#0B332B]/12 mb-5">
        <span class="text-[10px] font-bold tracking-[0.22em] uppercase text-[#0B332B]/60">
          How We Operate
        </span>
      </div>
      ...
      <p class="text-[#2A2A2A]/55 text-sm sm:text-base leading-relaxed max-w-xs">
        Safety, integrity, and operational excellence aren't values we aspire to — they're the standard on every consignment.
      </p>
    </div>
    ...
    <div class="hidden md:block relative" id="values-desktop">
      ...
      <div class="relative" style="z-index:2">
        ...
        @foreach($values as $i => [$num,$title,$desc,$side])
          ...
          <div class="flex {{ $isRight ? 'justify-end' : 'justify-start' }} mb-8"
            ...
            <div class="relative w-[52%] sm:w-[48%]"
                 data-values-card="{{ $i }}">
              ...
              <div class="absolute z-10 {{ $isRight ? '-top-2 left-5' : '-top-2 right-5' }}"
                   data-pin="{{ $i }}">
                {{-- Outer ring --}}
                <div class="w-5 h-5 rounded-full bg-[#1a1a1a] border-[3px] border-[#F0F0EF]
                            shadow-lg shadow-black/30 flex items-center justify-center">
                  {{-- Inner highlight --}}
                  <div class="w-2 h-2 rounded-full bg-white/30"></div>
                </div>
                {{-- Pin shaft shadow --}}
                <div class="absolute top-4 left-1/2 -translate-x-1/2 w-0.5 h-2
                            bg-black/15 rounded-b-full"></div>
              </div>
              ...
              <div class="bg-white rounded-3xl p-6
                ...
                <span class="block text-[10px] font-bold tracking-[0.22em]
                             text-[#F5850F]/70 mb-2">
                  {{ $num }}
                </span>
                ...
                <p class="text-[#2A2A2A]/60 text-[13px] leading-relaxed">
                  {{ $desc }}
                </p>
      ...
      {{-- End tag --}}
      <div class="flex items-center justify-end gap-2 -mt-2 pr-4
      ...
      <div class="relative pl-8">
        ...
        @foreach($values as $i => [$num,$title,$desc,$side])
          <div class="relative mb-8 last:mb-0">
            ...
            {{-- Card --}}
            <div class="relative bg-white rounded-3xl p-5
              ...
              <div class="absolute -top-1.5 left-5">
                <div class="w-3.5 h-3.5 rounded-full bg-[#1a1a1a] border-2 border-[#F0F0EF]
                            shadow-sm"></div>
              </div>
              ...
              <p class="text-[#2A2A2A]/60 text-[13px] leading-relaxed">{{ $desc }}</p>

            </div>
          </div>
        @endforeach

        <div class="flex items-center gap-2 pl-0 pt-1
                    text-[12px] font-semibold italic text-[#0B332B]/40">
    ...
    {{-- ── Leadership quote ── --}}
    <div class="mt-14 sm:mt-16 bg-white rounded-3xl p-7 sm:p-8
      ...
      <p class="font-display text-base sm:text-lg text-[#0B332B]/75 leading-relaxed italic mb-6">
        Lasting energy partnerships are built on consistently delivering what we promise — safely, on time, and with uncompromised product integrity.
      </p>
      <div class="flex items-center gap-3">
        <div class="w-9 h-9 rounded-full bg-[#F5850F] flex items-center justify-center
                    text-white font-bold text-sm shrink-0">AO</div>
        <div>
          <p class="text-[12px] font-bold text-[#0B332B]">Alms Oil Executive Leadership</p>
          <p class="text-[11px] text-[#F5850F]">Nigeria Energy Operations</p>
        </div>
      </div>
    </div>
...
(function () {
  function drawCurves() {
    ...
    var pins = Array.from(document.querySelectorAll('[data-pin]'))
      .sort(function(a,b){ return parseInt(a.dataset.pin) - parseInt(b.dataset.pin); })
      .map(function(el){
