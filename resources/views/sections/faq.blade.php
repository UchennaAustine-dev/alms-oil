      ...
      <div class="lg:col-span-4">
        <div class="lg:sticky lg:top-24 space-y-5">
          ...
          <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full
                      bg-[#0B332B]/5 border border-[#0B332B]/10">
          ...
          <p class="text-[#2A2A2A]/60 text-xs sm:text-sm leading-relaxed">
            Everything you need to know about our petroleum product specs, delivery lead times, PIA compliance, and commercial terms.
          </p>
          ...
          <div class="bg-[#0B332B] text-white rounded-3xl p-6 space-y-4 shadow-xl border border-white/10">
            <div class="w-10 h-10 rounded-2xl bg-[#F5850F]/20 flex items-center justify-center">
              <svg class="w-5 h-5 text-[#F5850F]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          ...
          @foreach($faqGroups as $gi => $group)
            <div>
              ...
              <div class="flex items-center gap-3 mb-3">
                <span class="text-[10px] font-bold uppercase tracking-[0.22em] text-[#F5850F]">
                  {{ $group[0] }}
                </span>
                ...
                @foreach($group[1] as $fi => $faq)
                  ...
                  <div class="faq-item rounded-2xl border border-[#0B332B]/10 bg-white
                    ...
                    <button class="faq-trigger w-full flex items-center justify-between gap-4
                            ...
                            data-target="{{ $key }}"
                            aria-expanded="false">
                      <span class="text-xs sm:text-sm font-bold text-[#0B332B] leading-snug">
                        {{ $faq[0] }}
                      </span>
                      {{-- +/− icon --}}
                      <span class="faq-icon w-7 h-7 rounded-full bg-[#0B332B]/5
                    ...
                    <div class="faq-body overflow-hidden"
                      ...
                      <p class="px-5 pb-5 sm:px-6 sm:pb-6
                                text-xs sm:text-[13px] text-[#2A2A2A]/65 leading-relaxed
  ...
  triggers.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      triggers.forEach(function (other) {
        ...
        if (otherIcon)  { otherIcon.textContent = '+'; otherIcon.style.background = ''; otherIcon.style.color = ''; }
