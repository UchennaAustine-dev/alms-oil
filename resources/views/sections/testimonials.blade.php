    ...
    $testimonialItems =
        isset($testimonials) && $testimonials->isNotEmpty()
            ...
            : collect([
                (object) [
                    'quote' =>
                        'Alms Oil has been our primary AGO supplier for 3 manufacturing facilities across Lagos and Ogun State. Product quality is guaranteed and delivery is always seamless.',
                    'name' => 'Engr. Babatunde Adeleke',
                    'role' => 'VP Operations',
                    'company' => 'Consolidated Manufacturing Ltd',
                    'initial' => 'B',
                ],
                (object) [
                    'quote' =>
                        'Their 24/7 telemetry fleet tracking gives us total transparency. When we needed emergency dispatch during grid outages, Alms Oil delivered within 4 hours.',
                    'name' => 'Dr. Chidi Nwachukwu',
                    'role' => 'Logistics Director',
                    'company' => 'West Coast Telecommunications',
                    'initial' => 'C',
                ],
                (object) [
                    'quote' =>
                        'From tank farm maintenance to fuel station supply, Alms Oil demonstrates true PIA compliance and regulatory professionalism. They are our long-term energy partner.',
                    'name' => 'Alhaji Farouk Bello',
                    'role' => 'Managing Director',
                    'company' => 'Sahara Retail Energy',
                    'initial' => 'F',
                ],
            ]);
...
<section class="bg-[#F0F0EF]/60 border-t border-[#0B332B]/8 overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 py-14 sm:py-20 lg:py-28">
        ...
        <div class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between mb-10 sm:mb-14">
            ...
            {{-- Rating chip --}}
            <div
                class="flex items-center gap-4 bg-white border border-[#0B332B]/8
                ...
                <div>
                    ...
                    <p class="text-[14px] font-bold text-[#0B332B] leading-none">5.0 / 5.0</p>
                    <p class="text-[14px] text-[#2A2A2A]/45 mt-0.5">Verified Deliveries</p>
                </div>
                <div class="w-px h-9 bg-[#0B332B]/10 shrink-0"></div>
                <div>
                    <p class="text-xl font-display font-black text-[#0B332B] leading-none">100%</p>
                    <p class="text-[14px] text-[#2A2A2A]/45 mt-0.5">Retention</p>
                </div>
            </div>
    ...
    ══════════════════════════ --}}
        <div class="flex flex-col gap-5 lg:grid lg:grid-cols-12 lg:items-stretch">
            ...
            <div
                class="lg:col-span-5 bg-[#0B332B] rounded-3xl p-6 sm:p-8
                ...
                <div class="relative z-10 mb-6">
                    ...
                    {{-- Big quote mark --}}
                    <div
                        class="font-display text-5xl sm:text-6xl text-[#F5850F]/35
                      leading-none mb-4 select-none">
                        &ldquo;</div>
                    {{-- Quote text --}}
                    <p class="text-white/85 text-[16px]">
                        {{ $first->quote }}
                    </p>
                </div>
                ...
                {{-- Author --}}
                <div
                    class="relative z-10 flex flex-wrap items-center gap-3
                    pt-6 border-t border-white/10">
                    <div
                        class="w-10 h-10 rounded-full bg-[#F5850F]
                      flex items-center justify-center
                      text-white font-bold text-[16px] shrink-0">
                        {{ $first->initial ?? substr($first->name, 0, 1) }}
                    </div>
                    <div class="min-w-0 flex-1">
                        <p class="text-[16px] font-bold text-white truncate">{{ $first->name }}</p>
                        <p class="text-[13px] text-white/50 mt-0.5 truncate">
            ...
            <div class="lg:col-span-7 flex flex-col gap-5">
                ...
                @foreach ($rest as $i => $t)
                    <div
                        class="flex-1 bg-white rounded-3xl p-5 sm:p-7
                      ...
                      flex flex-col justify-between">
                        ...
                        <div class="flex items-start justify-between mb-4">
                            ...
                            <div
                                class="font-display text-4xl text-[#F5850F]/18
                          leading-none select-none -mt-1 ml-2">
                                &ldquo;</div>
                        </div>

                        <p class="text-[#2A2A2A]/70 text-[16px] leading-relaxed flex-1 mb-5">
                            {{ $t->quote }}
                        </p>
                        ...
                        pt-4 border-t border-[#0B332B]/6">
                            <div class="flex items-center gap-3 min-w-0">
                                <div
                                    class="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-[#0B332B]
                            flex items-center justify-center
                            text-white font-bold text-xs shrink-0">
                                    {{ $t->initial ?? substr($t->name, 0, 1) }}
                                </div>
                                <div class="min-w-0">
                                    <p class="text-[16px] font-bold text-[#0B332B] truncate">{{ $t->name }}</p>
                                    <p class="text-[13px] text-[#2A2A2A]/45 mt-0.5 truncate">
                                        {{ $t->role }}, {{ $t->company }}
                                    </p>
                                </div>
                            </div>
