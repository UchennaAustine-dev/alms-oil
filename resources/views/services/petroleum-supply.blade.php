...
@section('description', 'Reliable bulk supply of AGO, PMS, DPK, Jet A-1, LPG and specialty products from certified
    depots across all 36 Nigerian states. NMDPRA licensed.')
    ...
    <section class="relative overflow-hidden bg-alms-green" style="min-height:82vh">
        ...
        <div class="absolute inset-0 z-0">
            <img src="https://images.pexels.com/photos/36228061/pexels-photo-36228061.jpeg?auto=compress&cs=tinysrgb&w=1600&q=85"
                alt="Petroleum storage tanks" class="w-full h-full object-cover object-center"
                style="filter:saturate(0.65) brightness(0.4) contrast(1.1)" />
            <div class="absolute inset-0"
                style="background:linear-gradient(135deg,rgba(11,51,43,0.88) 0%,rgba(11,51,43,0.5) 55%,rgba(11,51,43,0.72) 100%)">
            </div>
        </div>
            ...
            <div class="py-10 sm:py-14 lg:py-16 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-end">
                <div>
                    ...
                    <p class="text-white text-sm sm:text-base lg:text-lg leading-relaxed mb-7 sm:mb-8 max-w-lg">
                        NMDPRA-licensed bulk supply of AGO, PMS, DPK, Jet A-1, LPG, and specialty products from certified
                        depots across all 36 Nigerian states.
                    </p>
                ...
                <div class="space-y-3">
                    <div class="backdrop-blur-md bg-white/8 border border-white/15 rounded-3xl p-5 sm:p-6">
                        <p class="text-white text-sm sm:text-base leading-relaxed">
                            We connect industrial consumers, retailers, and government entities to certified petroleum
                            products — backed by a robust logistics network and strict quality assurance at every stage.
                        </p>
                    </div>
                    <div class="grid grid-cols-3 gap-3">
                        @foreach ([['9+', 'Product Types'], ['850M+', 'Litres Supplied'], ['36', 'States Active']] as [$v, $l])
                            <div
                                class="backdrop-blur-md bg-white/8 border border-white/12 rounded-2xl px-3 py-4 text-center">
                                <p class="font-display font-black text-white leading-none mb-1"
                                    style="font-size:clamp(1.3rem,2.2vw,1.75rem)">{{ $v }}</p>
                                <p class="text-[9px] font-bold uppercase tracking-[0.18em] text-white">{{ $l }}
                                </p>
                            </div>
                        @endforeach
                    </div>
    ...
    <section class="bg-[#F8FAFC] py-14 sm:py-20 lg:py-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10 sm:mb-14">
                ...
                <p class="text-base text-alms-charcoal/55 max-w-xs sm:text-right ">
            ...
            @php
                $products = [
                    [
                        ...
                        'desc' =>
                            'Diesel fuel for trucks, generators, and industrial equipment. Multiple grade specifications.',
                        'bg' => '#0B332B',
                        'icon' => 'truck',
                    ],
                    [
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
                @foreach ($products as $product)
                    <div
                        class="srv-reveal group bg-white rounded-2xl sm:rounded-3xl border border-[#0B332B]/10 p-5 sm:p-6 hover:border-[#F5850F]/40 hover:shadow-lg hover:shadow-[#0B332B]/8 transition-all duration-300">
                        <div class="flex items-start gap-3.5 mb-4">
                            <svg class="w-9 h-9 shrink-0 mt-0.5" style="color: {{ $product['bg'] }}" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6" aria-hidden="true">
                                @switch($product['icon'])
                                    @case('truck')
                                    ...
                                    @case('pump')
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M8 3h5a2 2 0 012 2v12a1 1 0 01-1 1H8a1 1 0 01-1-1V4a1 1 0 011-1z" />
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M15 8h1.5A2.5 2.5 0 0119 10.5V13a1 1 0 01-1 1h-1" />
                                    ...
                                    @case('flame')
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M12 3c0 3-2.5 4.5-2.5 7.5A2.5 2.5 0 0012 13a2.5 2.5 0 002.5-2.5C14.5 7.5 12 6 12 3z" />
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M8.5 12.5c-1.5 1.2-2.5 3-2.5 5a6 6 0 0012 0c0-2-1-3.8-2.5-5" />
                                    @break

                                    @case('plane')
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M3.5 12.5l16.5-7-5.5 15-3.5-5.5-5.5-2.5z" />
                                    ...
                                    @case('cylinder')
                                        ...
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M10 4.5V6a1 1 0 01-1 1H8.5A2.5 2.5 0 006 9.5v8A2.5 2.5 0 008.5 20h7a2.5 2.5 0 002.5-2.5v-8A2.5 2.5 0 0015.5 7H15a1 1 0 01-1-1V4.5" />
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6M9 15.5h6" />
                                    @break

                                    @case('droplet')
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M12 3.5S6.5 9.2 6.5 13.5a5.5 5.5 0 0011 0C17.5 9.2 12 3.5 12 3.5z" />
                            ...
                            <div>
                                <span
                                    class="text-[11px] font-black uppercase tracking-widest text-[#F5850F]">{{ $product['code'] }}</span>
                                <h3 class="text-sm sm:text-base font-bold text-[#0B332B] leading-snug">
                                    {{ $product['name'] }}</h3>
                            </div>
                        </div>
                        <p class="text-[16px] text-[#2A2A2A]/60">{{ $product['desc'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-center">
                ...
                <div class="relative srv-reveal order-2 lg:order-1">
                    <div
                        ...
                        <div
                            class="absolute inset-0 bg-linear-to-t from-alms-green/50 to-transparent rounded-2xl sm:rounded-3xl">
                        </div>
                    </div>
                    <div
                        ...
                        <div>
                            <p class="text-[10px] font-extrabold uppercase tracking-wider text-alms-orange">Quality
                                Certified</p>
                ...
                <div class="order-1 lg:order-2 srv-reveal">
                    ...
                    <div class="space-y-4">
                        @foreach ([['Supply Volumes', 'From 5,000 L retail orders to 5M+ L industrial contracts. Flexible volume tiers structured around your operations.'], ['Quality Assurance', 'DPR-certified supply chain. Every product batch is lab-tested before dispatch — no shortcuts.'], ['Nationwide Coverage', 'Active supply routes in all 36 states and FCT. Same-day capability in Lagos, Abuja, Port Harcourt.'], ['Flexible Contracts', 'Spot supply, monthly retainer, or long-term trading agreements. We structure around your cash flow.']] as [$title, $desc])
                            <div
                                class="flex gap-4 p-4 sm:p-5 rounded-2xl bg-[#F8FAFC] border border-alms-green/8 hover:border-alms-orange/30 hover:bg-alms-orange/2 transition-all duration-200">
                                
                                <div>
                                    <h3 class="font-bold text-alms-green mb-1">{{ $title }}</h3>
                                    <p class=" text-alms-charcoal/60 ">{{ $desc }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
    ...
    <section class="bg-[#F8FAFC] py-12 sm:py-16 border-t border-alms-green/8">
        <div class="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                @foreach ([['Logistics & Haulage', 'GPS-tracked fleet, nationwide delivery.', route('services.logistics')], ['Engineering Services', 'Installation, maintenance & HSE compliance.', route('services.engineering')], ['Energy Infrastructure', 'Tank farms, depots & filling stations.', route('services.infrastructure')]] as [$t, $s, $r])
                    <a href="{{ $r }}"
                        class="group flex items-center justify-between gap-4 p-4 sm:p-5 rounded-2xl bg-white border border-alms-green/8 hover:border-alms-orange/40 hover:shadow-md transition-all duration-200">
                        <div>
                            <h3 class="text-sm font-bold text-alms-green group-hover:text-alms-orange transition-colors">
                                {{ $t }}</h3>
                            <p class="text-[13px] text-alms-charcoal/50 mt-0.5">{{ $s }}</p>
                        </div>
